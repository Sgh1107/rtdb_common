...
struct HeapRegion {
    explicit HeapRegion(uint64_t bytes)
        : size_(bytes), mem_(static_cast<char*>(::operator new(bytes, std::align_val_t(4096)))) {
...
TEST_F(SlabTest, RejectZeroAndOversize) {
    ...
    EXPECT_EQ(a.Value()->Allocate(64ull * 1024 + 1).Error(), rtdb::Err::kUnsupported);
}
...
TEST_F(SlabTest, MultiClassDistinctAlignedOffsets) {
    ...
    for (uint64_t sz : {32u, 64u, 256u, 1024u, 8192u}) {
        ...
        *reinterpret_cast<uint64_t*>(a.Value()->OffsetToPtr(r.Value())) = r.Value();
    }
    for (auto off : seen)
        EXPECT_EQ(*reinterpret_cast<const uint64_t*>(a.Value()->OffsetToPtr(off)), off);
}
