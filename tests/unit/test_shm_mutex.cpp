    ...
    for (int t = 0; t < kThreads; ++t) {
        pool.emplace_back([&] {
            for (int i = 0; i < kIters; ++i) {
                ...
                while (d > cur &&
                       !max_depth.compare_exchange_weak(cur, d, std::memory_order_relaxed)) {
                }
                shared += 1;
...
TEST(ShmMutexDeathRecovery, RecoversAfterHolderHardKill) {
    ...
    const std::string img =
        (fs::temp_directory_path() /
         ("rtdb_crash_lock_" + std::to_string(platform::CurrentProcessId()) + ".rtdb"))
            .string();
    std::error_code ec;
    fs::remove(img, ec);
    ...
    auto res = rtdb::Engine::Open(opts);
    ASSERT_TRUE(res.IsOk()) << "reopen after crash: " << rtdb::ErrorMessage(res.Error());
