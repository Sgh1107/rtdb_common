...
struct HostRegion {
    HostRegion() : mem(static_cast<char*>(::operator new(RegionBytes, std::align_val_t(4096)))) {
...
struct BTreeEnv {
    ...
    BTreeEnv()
        : region(), alloc(infra::SlabShmAllocator::Attach(region.mem, kBytes, true).TakeValue()) {}
    ...
    std::unique_ptr<infra::SlabShmAllocator> Reattach() {
        return infra::SlabShmAllocator::Attach(region.mem, kBytes, false).TakeValue();
    }
...
TEST(SharedBTreeInt, ModelDiffProbe) {
    ...
    for (int step = 0; step < N; ++step) {
        ...
        for (int p : probes) {
            ...
            EncodeKey(p, pk);
            if (!tree.Find(pk, &out)) {
                std::printf("[model] FIND MISS at step=%d probed=%d lastKey=%d\n", step, p, k);
                FAIL();
            }
        }
        (void)kb;
...
TEST(SharedBTreeInt, StressSplitsAndValidation) {
    ...
    for (int n = 0; n < N; ++n) {
        char k[8];
        EncodeKey(idx[n], k);
        ASSERT_EQ(tree.Insert(k, static_cast<uint64_t>(idx[n])), rtdb::Err::kOk) << "at " << n;
...
TEST(SharedBTreeInt, ReattachSeesSameContent) {
    ...
    IntTree::Slot slot{0, 0};
    {
        ...
        uint64_t visited = 0;
        ASSERT_EQ(tree.Validate(&visited), rtdb::Err::kOk);  // 写入侧结构不变量
    }
    ...
    if (!missing.empty()) {
        ...
        std::printf("[diag] missing=%zu first={idx=%d key=%d} trace=%s\n", missing.size(),
                    missing.front(), missing.front() * 3, trace.c_str());
        // 游标全表计数对照
        int seen = 0;
        char kb2[8], hb2[8];
        EncodeKey(0, kb2);
