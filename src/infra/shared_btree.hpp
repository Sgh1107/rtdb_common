...
public:
    ...
    static_assert(std::is_trivially_copyable<Payload>::value, "载荷必须可平凡拷贝");
    ...
    SharedBTree(void* base_addr, SlabShmAllocator* alloc, Slot* slot) noexcept
        ...
        while (true) {
            ...
            if (n->leaf != 0) {
                ...
                if (pos < n->count && KeyCmpAt(off, pos, key) == 0) {
                    if (out != nullptr) std::memcpy(out, LeafValuePtr(off, pos), sizeof(Payload));
                    return true;
                }
                return false;
            }
    ...
    bool DebugFindTrace(const void* key, std::string* log) const noexcept {
        ...
        while (true) {
            const NodeHeader* n = Node(off);
            const int pos =
                n->leaf != 0 ? LowerBound(off, key, n->count) : StrictUpper(off, key, n->count);
            (*log) += "[node@";
            append_num(off);
            (*log) += " cnt=";
            append_num(n->count);
            ...
            if (pos < n->count) {
                (*log) += " key[pos]@";
                append_num(KeyCmpAt(off, pos, key));
                (*log) += "]";
            } else {
                (*log) += " key[pos]=END]";
            }
            if (n->leaf != 0) {
                const bool hit = pos < n->count && KeyCmpAt(off, pos, key) == 0;
                return hit;
            }
            off = ChildAt(off, pos);
        }
    ...
    uint64_t DebugNextLeaf(uint64_t leaf_off) const noexcept { return Node(leaf_off)->next_leaf; }
    ...
    Err Insert(const void* key, const Payload& value) noexcept {
        ...
        if (Node(root)->count >= CapOf(Node(root))) {
            ...
            Result<uint64_t> nr = AllocNode(0, static_cast<uint8_t>(Node(root)->level + 1));
        ...
        while (true) {
            ...
            if (n->leaf != 0) {
                ...
                if (pos < n->count && KeyCmpAt(cur, pos, key) == 0) return Err::kAlreadyExists;
    ...
    Err Remove(const void* key) noexcept {
        ...
        while (true) {
            ...
            if (n->leaf != 0) {
                const int pos = LowerBound(off, key, n->count);
                if (!(pos < n->count && KeyCmpAt(off, pos, key) == 0)) return Err::kNotFound;
    ...
    private:
        ...
        Cursor(const SharedBTree* t, const void* hi, bool hi_incl) noexcept
            : tree_(t), hi_incl_(hi_incl), valid_(false), leaf_(0), idx_(0), val_(nullptr) {
    ...
    Cursor SeekRange(const void* lo, bool lo_incl, const void* hi, bool hi_incl) const noexcept {
        ...
        while (true) {
            const NodeHeader* n = Node(off);
            const int pos =
                n->leaf != 0 ? LowerBound(off, lo, n->count) : StrictUpper(off, lo, n->count);
            if (n->leaf != 0) {
                cur.leaf_ = off;
...
private:
    ...
    static constexpr size_t kLeafStride = detail::AlignUp8(MaxKeyLen + sizeof(Payload));
    static constexpr size_t kIntStride = detail::AlignUp8(MaxKeyLen + 8);
    static constexpr int kCapLeaf = static_cast<int>((kNodeBytes - kHdr) / kLeafStride);
    static constexpr int kCapInt = static_cast<int>((kNodeBytes - kHdr - 8) / kIntStride);
    ...
    const NodeHeader* Node(uint64_t off) const noexcept { return PtrOf<NodeHeader>(off); }
    NodeHeader* MutHeader(uint64_t off) const noexcept { return PtrOf<NodeHeader>(off); }
    static int CapOf(const NodeHeader* n) noexcept { return n->leaf != 0 ? kCapLeaf : kCapInt; }
    ...
    char* KeyPtrAt(uint64_t off, size_t i) const noexcept {
        const NodeHeader* n = Node(off);
        return static_cast<char*>(base_) + off + (n->leaf != 0 ? kHdr : kHdr + 8) +
    ...
    uint64_t ChildAt(uint64_t off, int i) const noexcept {
        ...
        uint64_t c;
        std::memcpy(&c, KeyPtrAt(off, static_cast<size_t>(i - 1)) + MaxKeyLen, 8);
        return c;
    }
    /// 写内部节点第 i 个实体（键 + 右孩），要求 i >= 1。
    void SetChildEntry(uint64_t off, int i, const void* key, uint64_t child) const noexcept {
    ...
    Result<uint64_t> AllocNode(uint8_t leaf, uint8_t level) const noexcept {
        Result<uint64_t> r = alloc_->Allocate(static_cast<uint64_t>(kNodeBytes));
        if (!r.IsOk()) return r;
        // 新节点整页清零：未用槽位/保留字段确定性归零，使映像内容与
        // 分配历史无关（"按 count 读取"的正确性前提，也利于持久一致）。
        std::memset(PtrOf<void>(r.Value()), 0, static_cast<size_t>(kNodeBytes));
    ...
    void ShiftEntriesRight(uint64_t off, int pos, int n_after) const noexcept {
        ...
        char* start = static_cast<char*>(base_) + off + (h->leaf != 0 ? kHdr : kHdr + 8) +
    ...
    void ShiftEntriesRange(uint64_t src_node, int sx, int ex, uint64_t dst_node,
    ...
    Err SplitChild(uint64_t parent, int idx, uint64_t child) noexcept {
        ...
        if (ch->leaf != 0) {
            ...
            for (int j = mid + 1; j < ch->count; ++j) {
                ...
                std::memcpy(&c, KeyPtrAt(child, static_cast<size_t>(j)) + MaxKeyLen, 8);
    ...
    void LoadAndCheckHi(Cursor* c) const noexcept {
        while (true) {
            ...
            std::memcpy(c->key_, RawKeyAt(c->leaf_, static_cast<int>(c->idx_)), MaxKeyLen);
