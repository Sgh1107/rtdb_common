...
Err ValidateSuperBlock(const shm_SuperBlock& sb) noexcept {
    ...
    fields->layout_crc = 0;
    if (Crc32(snapshot, kStableFieldsBytes) != expected_crc) return Err::kIncompatibleLayout;
    return Err::kOk;
}
...
Result<std::unique_ptr<ShmRegion>> ShmRegion::Open(const ShmRegionParams& p) {
    ...
    if (!file_res.IsOk()) return Result<std::unique_ptr<ShmRegion>>::Fail(file_res.Error());
    ...
    const bool header_all_zero = std::memcmp(sb, kZeroHeader, sizeof(shm_SuperBlock)) == 0;
    ...
    if (p.role != Role::Reader) {
        auto alloc_res =
            infra::SlabShmAllocator::Attach(region->base_, region->mapped_bytes(), header_all_zero);
        if (!alloc_res.IsOk()) return Result<std::unique_ptr<ShmRegion>>::Fail(alloc_res.Error());
        region->alloc_ = alloc_res.TakeValue();

        auto* ms = reinterpret_cast<infra::shm_RootMutexState*>(static_cast<char*>(region->base_) +
                                                                kRootMutexOffset);
...
uint64_t ShmRegion::size_bytes() const noexcept { return super_block_->max_size_bytes; }

int64_t ShmRegion::LoadCommittedSeq() const noexcept {
    return static_cast<int64_t>(AsAtomicU64(&super_block_->committed_seq)->load());
}
...
Err ShmRegion::Flush() noexcept { return file_->flush(base_, kHeaderReservedBytes); }
