...
constexpr uint64_t AlignUp64(uint64_t v, uint64_t a) noexcept { return (v + a - 1) / a * a; }

constexpr uint64_t AlignDown64(uint64_t v, uint64_t a) noexcept { return v / a * a; }

}  // namespace

Result<std::unique_ptr<SlabShmAllocator>> SlabShmAllocator::Attach(void* base_addr,
                                                                   uint64_t map_size,
                                                                   bool create_new) {
    if (base_addr == nullptr || map_size < kDataStartOffset)
        return Result<std::unique_ptr<SlabShmAllocator>>::Fail(Err::kInvalidArgument);

    auto* ctl = reinterpret_cast<shm_AllocCtl*>(static_cast<char*>(base_addr) + kAllocCtlOffset);
    ...
    if (!create_new && ctl->class_count != kSlabClassCount)
        return Result<std::unique_ptr<SlabShmAllocator>>::Fail(Err::kIncompatibleLayout);
...
Result<uint64_t> SlabShmAllocator::Allocate(uint64_t size) noexcept {
    ...
    if (ctl_->free_heads[cls] != 0) {
        ...
        const uint64_t* cell = static_cast<const uint64_t*>(OffsetToPtr(user_off));
        ctl_->free_heads[cls] = *cell;
    } else {
        // 无空闲则从 bump 线性切分：用户区落在级尺寸边界上，
        // 前置头部空间不破坏用户对齐；真实跨度（含间隙）入账。
        const uint64_t off = AlignUp64(ctl_->bump + kHdrBytes, cls_size > 8 ? cls_size : 8);
        if (off + cls_size > ctl_->data_end) return Result<uint64_t>::Fail(Err::kOutOfMemory);
...
Err SlabShmAllocator::Deallocate(uint64_t user_off) noexcept {
    ...
    *static_cast<uint64_t*>(OffsetToPtr(user_off)) = ctl_->free_heads[hdr->class_idx];
    ctl_->free_heads[hdr->class_idx] = user_off;
    return Err::kOk;
}

bool SlabShmAllocator::OwnsOffset(uint64_t user_off) const noexcept {
    return user_off >= ctl_->data_base + kHdrBytes && user_off + 8 <= ctl_->data_end &&
           (user_off % 8) == 0;
}
