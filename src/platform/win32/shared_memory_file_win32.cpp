...
std::wstring Utf8ToWide(const std::string& s) noexcept {
  ...
  MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, s.c_str(),
                      static_cast<int>(s.size()), &wide[0], need);
  return wide;
}
...
class WinSharedMemoryFile final : public SharedMemoryFile {
 public:
  ...
  static Result<std::unique_ptr<SharedMemoryFile>> Open(
      const ShmParams& p) {
    ...
    HANDLE file = CreateFileW(wpath.c_str(), desired_access, share_mode,
                              nullptr, creation, FILE_ATTRIBUTE_NORMAL, nullptr);
    if (file == INVALID_HANDLE_VALUE)
      return Result<std::unique_ptr<SharedMemoryFile>>::Fail(
          LastErrorToErr(GetLastError()));

    // CreateFileMapping 会自动把零长度文件扩展到 dwMaximumSizeLow，
    // 因此无需手动 SetFilePointer/SetEndOfFile。
    HANDLE mapping = CreateFileMappingW(file, nullptr,
                                        p.readonly ? PAGE_READONLY : PAGE_READWRITE,
                                        static_cast<DWORD>(p.size_bytes >> 32),
                                        static_cast<DWORD>(p.size_bytes & 0xFFFFFFFFu),
